...
def formatRow(result, form, config):
    ...
    try:
        ...
        csid = 'user'
    link = protocol + '://' + hostname + port + '/cspace/' + institution.replace('botgarden', 'ucbg') +  '/record/all/%s' % csid

    # the link to acquisitions is for PAHMA...
    try:
        link2 = protocol + '://' + hostname + port + '/cspace/' + institution.replace('botgarden', 'ucbg') +  '/record/all/%s' % rr[24]
    except:
        link2 = ''
    ...
    try:
        ...
        csid = 'user'
    link3 = protocol + '://' + hostname + port + '/cspace/' + institution.replace('botgarden', 'ucbg') +  '/record/all/%s' % csid
    ...
    if result['rowtype'] == 'subheader':
        ...
        link = protocol + '://' + hostname + port + '/cspace/' + institution.replace('botgarden', 'ucbg') +  '/record/all/%s' % rr[7]
        if groupby == 'none':
        ...
        link = protocol + '://' + hostname + port + '/cspace/' + institution.replace('botgarden', 'ucbg') +  '/record/all/%s' % rr[6]
        ...
        if institution == 'bampfa':
            ...
            link = protocol + '://' + hostname + port + '/cspace/ucbg/record/all/%s' % rr[7]
            return """<tr><td class="objno"><a target="cspace" href="%s">%s</a></td><td class="objname">%s</td>
            <td class="rdo" ><input type="radio" id="sel-move" name="r.%s" value="move|%s|%s|%s|%s|%s"></td>
            <td class="rdo" ><input type="radio" id="sel-nomove" name="r.%s" value="do not move|%s|%s|%s|%s|%s" checked/></td></tr>""" % (
                link, rr[4], rr[5],
                rr[4], rr[7], rr[13], '', rr[4], '',
                rr[4], rr[7], rr[13], '', rr[4], '')
