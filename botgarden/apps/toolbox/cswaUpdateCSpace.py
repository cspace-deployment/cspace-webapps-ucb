...
def updateLocations(updateItems, config, form):
    ...
    if when2post == 'update':
        # for UCBG, "moves" simply update existing Movement record.
        # makeMH2R(updateItems, config, form)
        updateMovement(updateItems, config, form)
        return 'moved', ''
    elif when2post == 'queue':
...
def updateMovement(updateItems, config, form):
    uri = 'movements' + '/' + updateItems['movementCsid']
    writeLog(updateItems, uri, 'PUT', form, config)
    ...
    if httpcode != 200:
        sys.stderr.write("ERROR: HTTP response code %s for GET %s\n" % (httpcode, url))
        sys.stderr.write("ERROR: content for %s: \n%s\n" % (url, content))
        return "ERROR: HTTP response code %s for GET %s\n" % (httpcode, url)
    if content is None:
        sys.stderr.write("ERROR: No XML returned from CSpace server for %s\n" % url)
        return "ERROR: No XML returned from CSpace server for %s\n" % url
    try:
        message, payload = updateMHXML(updateItems, content)
    except:
        try:
            sys.stderr.write("ERROR: payload for PUT %s: \n%s\n" % (url, payload))
        except:
            sys.stderr.write("ERROR: could not write payload to error log for %s: \n" % url)
        return "ERROR generating update XML\n"
    try:
        (url3, data, httpcode, elapsedtime) = postxml('PUT', uri, payload, form)
    except:
        sys.stderr.write("ERROR: failed PUT payload for %s: \n%s\n" % (url, payload))
    if data is None:
        sys.stderr.write("ERROR: failed PUT payload for %s: \n%s\n" % (url, payload))
        sys.stderr.write("ERROR: HTTP response code %s for  %s\n" % (httpcode, url))
        return "ERROR: Bad HTTP response code %s for  %s\n" % (httpcode, url)
    # sys.stderr.write("payload for %s: \n%s" % (url, payload))
    # sys.stderr.write("updated object with csid %s to REST API...\n" % updateItems['objectCsid'])
    return message


def updateMHXML(updateItems, xml):
    message = ''
    root = etree.fromstring(xml)

    currentLocation = root.find('.//currentLocation')
    currentLocation.text = updateItems['toRefname']

    # move the current location to the previous location, if there is one.
    try:
        previousLocation = root.find('.//previousLocation')
        previousLocation.text = currentLocation.text
    except:
        pass
    ...
    reasonForMove = root.find('.//reasonForMove')
    reasonForMove.text = "urn:cspace:botgarden.cspace.berkeley.edu:vocabularies:name(actionCode):item:name(actCode02)'Moved'"

    locationDate = root.find('.//locationDate')
    locationDate.text = updateItems['locationDate']
