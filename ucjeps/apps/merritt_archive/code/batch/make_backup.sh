...
echo aws s3 cp --no-progress /tmp/${DB_NAME}.backup.${RUN_DATE}.gz ${S3URI}/${DB_NAME}.backup.${RUN_DATE}.gz
aws s3 cp --no-progress /tmp/${DB_NAME}.backup.${RUN_DATE}.gz ${S3URI}/${DB_NAME}.backup.${RUN_DATE}.gz
